...
- [x] **Logo files.** `assets/logo/wordmark.webp` added (cropped from a higher-resolution photo of the printed logo card) and wired into the header `<img>` on all pages. It's a raster crop of a photo, not a vector — if a clean SVG/high-res version of the logo becomes available, swap it in for sharper rendering at large sizes.
